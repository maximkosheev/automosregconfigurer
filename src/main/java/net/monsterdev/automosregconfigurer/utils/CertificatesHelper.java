...
public class CertificatesHelper {
     ...
     */
    public CertPath getCertPath(List<Certificate> rootCerts, Certificate cert) throws ConfigurerException {
        try {
            ...
            final PKIXBuilderParameters pkixBuilderParameters = new PKIXBuilderParameters(trust, selector);
            ...
            final CollectionCertStoreParameters ccsp = new CollectionCertStoreParameters(rootAndUserCerts);
    ...
    /**
     * Добавляет сертификат в доверенное хранилище сертификатов Java.
     * Добавление выполняется только в том случае, если добавляемого сертификата еще нет в хранилице
    ...
    public String getJTKSPath(@NonNull String jreHome) {
        ...
        return  (jreHome + "lib/security/cacerts").replaceAll("/", Matcher.quoteReplacement(File.separator));
    }
}
